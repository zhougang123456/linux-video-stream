...
void red_drawable_get(RedDrawable* red_drawable, SpiceRect* rect, unsigned char* data, unsigned int time)
{
    ...
    unsigned int height = red_drawable->u.copy.src_area.bottom;
    int width = red_drawable->u.copy.src_area.right;
    int format = BITMAP_FMT_32BIT;
    ...
    red_drawable->u.copy.src_bitmap->descriptor.id = (((unsigned long)0) << 32)
        | (((unsigned int)IMAGE_GROUP_DRIVER_DONT_CACHE << 30) | (((unsigned int)((width) & 0x1FFF)
            | ((unsigned int)((height) & 0x1FFF) << 13) | ((unsigned int)(format) << 26))));
