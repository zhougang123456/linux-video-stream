...
/* allocate window fields (no reset)*/
 bool SharedDictionary::glz_dictionary_window_create(unsigned int size)
{
    ...
    window.encoders_heads = (unsigned int*)cur_usr->malloc(cur_usr,sizeof(unsigned int) * max_encoders);
...
unsigned int SharedDictionary::glz_enc_dictionary_get_size(void)
{
...
{
    unsigned int cur_win_size;
	WindowImage* cur_head;

    if ((unsigned int)new_image_size > window.size_limit) {
...
{
    unsigned int old_free_head = window.free_segs_head;
    unsigned int seg_id, next_seg_id;
...
void SharedDictionary::glz_dictionary_window_remove_head(unsigned int encoder_id, WindowImage* end_image)
{
...
void SharedDictionary::glz_dictionary_window_segs_realloc(void)
{
    ...
    unsigned int new_quota = (MAX_IMAGE_SEGS_NUM < (window.segs_quota * 2)) ? MAX_IMAGE_SEGS_NUM : (window.segs_quota * 2);
	WindowImageSegment* seg;
    unsigned int i;

    pthread_rwlock_wrlock(&rw_alloc_lock);
    ...
    pthread_rwlock_unlock(&rw_alloc_lock);
}

/* NOTE - it doesn't update the used_segs list*/
unsigned int SharedDictionary::glz_dictionary_window_alloc_image_seg_impl(void)
{
    unsigned int seg_id;
	WindowImageSegment* seg;
...
unsigned int SharedDictionary::glz_dictionary_window_alloc_image_seg(WindowImage* image,
	int size, int stride,
    unsigned char* lines, unsigned int num_lines)
{
    unsigned int seg_id = glz_dictionary_window_alloc_image_seg_impl();
	...
	GlzUsrImageContext* usr_image_context)
{
	...
	unsigned int row;
    unsigned int seg_id;
    unsigned int prev_seg_id = 0;
    unsigned char* lines = first_lines;
	...
	for (row = 0;;) {
		...
		row += num_lines;
        if (row < (unsigned int)image_height) {
			num_lines = cur_usr->more_lines(cur_usr, &lines);
			if (num_lines <= 0) {
				//printf("more lines failed\n");
			}
...
WindowImage* SharedDictionary::glz_dictionary_pre_encode(unsigned int encoder_id, GlzEncoderUsrContext* usr,
    ...
    unsigned int* image_head_dist)
{
	...
	if (new_win_head) {
        ...
        *image_head_dist = (unsigned int)(ret->id - new_win_head->id); // shouldn't be greater than 32
    ...
    pthread_rwlock_rdlock(&rw_alloc_lock);
	return ret;
}

void SharedDictionary::glz_dictionary_post_encode(unsigned int encoder_id, GlzEncoderUsrContext* usr)
{
    unsigned int i;
    unsigned int early_head_seg = NULL_IMAGE_SEG_ID;
    unsigned int this_encoder_head_seg;
...
SharedDictionary::SharedDictionary(unsigned int size, unsigned int max_encoders_num, GlzEncoderUsrContext* usr)
{
    ...
    pthread_rwlock_init(&rw_alloc_lock, NULL);
	window.encoders_heads = NULL;
...
SharedDictionary::~SharedDictionary()
{
    ...
    pthread_rwlock_destroy(&rw_alloc_lock);
}
