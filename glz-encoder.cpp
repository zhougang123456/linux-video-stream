...
bool GlzEncoder::init(unsigned char id, SharedDictionary* dictionary, GlzEncoderUsrContext* usr_ctx)
{
...
bool GlzEncoder::encoder_reset(unsigned char* io_ptr, unsigned char* io_ptr_end)
{
...
void GlzEncoder::encode(unsigned char byte)
{
	if (io.now == io.end) {
		if (more_io_bytes() <= 0) {
...
{
    encode((unsigned char)(word >> 24));
    encode((unsigned char)(word >> 16) & 0x0000ff);
    encode((unsigned char)(word >> 8) & 0x0000ff);
    encode((unsigned char)(word & 0x0000ff));
}

void GlzEncoder::encode_64(unsigned long word)
{
    encode_32((unsigned int)(word >> 32));
    encode_32((unsigned int)(word & 0xffffffffu));
}

void GlzEncoder::encode_copy_count(unsigned char copy_count)
{
	encode(copy_count);
	io.last_copy = io.now - 1; // io_now cannot be the first byte of the buffer
}

void GlzEncoder::update_copy_count(unsigned char copy_count)
{
	if (!(io.last_copy)) {
		return;
	}
...
static inline int get_encode_ref_size(unsigned int image_distance, size_t pixel_distance)

{
	...
	if (pixel_distance < MAX_PIXEL_SHORT_DISTANCE) {
        ...
        unsigned char long_dist_control = (pixel_distance < MAX_PIXEL_MEDIUM_DISTANCE) ? 0 : 32;
		if (image_distance == 0) {
			encode_size = 3;
		}
...
void GlzEncoder::encode_match(unsigned int image_distance, size_t pixel_distance, size_t len)

{
	/* encoding the match length + Long/Short dist bit +  12 LSB pixels of pixel_distance*/
	if (len < 7) {
		if (pixel_distance < MAX_PIXEL_SHORT_DISTANCE) {
            encode((unsigned char)((len << 5) + (pixel_distance & 0x0f)));
		}
		else {
            encode((unsigned char)((len << 5) + 16 + (pixel_distance & 0x0f)));
		}
        encode((unsigned char)((pixel_distance >> 4) & 255));
	}
	else {
		if (pixel_distance < MAX_PIXEL_SHORT_DISTANCE) {
            encode((unsigned char)((7 << 5) + (pixel_distance & 0x0f)));
		}
		else {
            encode((unsigned char)((7 << 5) + 16 + (pixel_distance & 0x0f)));
		}
		for (len -= 7; len >= 255; len -= 255) {
			encode(255);
		}
        encode((unsigned char)len);
        encode((unsigned char)((pixel_distance >> 4) & 255));
	}
	/* encoding the rest of the pixel distance and the image_dist and its 2 control bits */
	/* The first 2 MSB bits indicate how many more bytes should be read for image dist */
	if (pixel_distance < MAX_PIXEL_SHORT_DISTANCE) {
		if (image_distance < SHORT_PIX_IMAGE_DIST_LEVEL_1) {
            encode((unsigned char)(image_distance & 0x3f));
		}
		else if (image_distance < SHORT_PIX_IMAGE_DIST_LEVEL_2) {
            encode((unsigned char)((1 << 6) + (image_distance & 0x3f)));
            encode((unsigned char)((image_distance >> 6) & 255));
		}
		else if (image_distance < SHORT_PIX_IMAGE_DIST_LEVEL_3) {
            encode((unsigned char)((1 << 7) + (image_distance & 0x3f)));
            encode((unsigned char)((image_distance >> 6) & 255));
            encode((unsigned char)((image_distance >> 14) & 255));
		}
		else {
            encode((unsigned char)((1 << 7) + (1 << 6) + (image_distance & 0x3f)));
            encode((unsigned char)((image_distance >> 6) & 255));
            encode((unsigned char)((image_distance >> 14) & 255));
            encode((unsigned char)((image_distance >> 22) & 255));
		}
	}
	else {
		/* the third MSB bit indicates if the pixel_distance is medium/long*/
        unsigned char long_dist_control = (pixel_distance < MAX_PIXEL_MEDIUM_DISTANCE) ? 0 : 32;
		if (image_distance == 0) {
            encode((unsigned char)(long_dist_control + ((pixel_distance >> 12) & 31)));
		}
		else if (image_distance < FAR_PIX_IMAGE_DIST_LEVEL_1) {
            encode((unsigned char)(long_dist_control + (1 << 6) + ((pixel_distance >> 12) & 31)));
            encode((unsigned char)(image_distance & 255));
		}
		else if (image_distance < FAR_PIX_IMAGE_DIST_LEVEL_2) {
            encode((unsigned char)(long_dist_control + (1 << 7) + ((pixel_distance >> 12) & 31)));
            encode((unsigned char)(image_distance & 255));
            encode((unsigned char)((image_distance >> 8) & 255));
		}
		else {
            encode((unsigned char)(long_dist_control + (1 << 7) + (1 << 6) +
				((pixel_distance >> 12) & 31)));
            encode((unsigned char)(image_distance & 255));
            encode((unsigned char)((image_distance >> 8) & 255));
            encode((unsigned char)((image_distance >> 16) & 255));
		}

		if (long_dist_control) {
            encode((unsigned char)((pixel_distance >> 17) & 255));
		}
	}
}
...
*/
void GlzEncoder::compress_seg(unsigned int seg_idx, PIXEL* from, int copied)
{
...
void GlzEncoder::glz_rgb32_compress(void)
{
    ...
    unsigned int seg_id = cur_image.first_win_seg;
	PIXEL* ip;
	int hval;
	...
	while ((seg_id != NULL_IMAGE_SEG_ID) &&
		...
		if (dict->window.segs[seg_id].lines != dict->window.segs[seg_id].lines_end) {
            ...
            encode_copy_count((unsigned char)((((PIXEL*)dict->window.segs[seg_id].lines_end)
	...
	GlzEncDictImageContext** o_enc_dict_context)
{
    ...
    unsigned char* io_ptr_end = io_ptr + num_io_bytes;
    unsigned int win_head_image_dist = 0;
