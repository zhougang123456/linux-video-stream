...
struct ImageEncoderSharedData {
    unsigned int glz_drawable_count;
};
...
struct RedCompressBuf {
	 ...
	 */
	union {
        unsigned char  bytes[RED_COMPRESS_BUF_SIZE];
        unsigned int words[RED_COMPRESS_BUF_SIZE / 4];
	} buf;
};
...
public:
    ...
    bool image_encoders_get_glz_dictionary(unsigned char id, int window_size);
    bool image_encoders_glz_create(unsigned char id);
...
private:
    ...
    GlzSharedDictionary* create_glz_dictionary(unsigned char id, int window_size);
