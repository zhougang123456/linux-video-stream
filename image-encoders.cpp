 ...
 * The pointer returned in io_ptr is garanteed to be aligned to 4 unsigned chars.
 */
static int encoder_usr_more_space(EncoderData* enc_data, unsigned char** io_ptr)
{
	RedCompressBuf* buf;
...
static inline int encoder_usr_more_lines(EncoderData* enc_data, unsigned char** lines)
{
...
static int glz_usr_more_space(GlzEncoderUsrContext* usr, unsigned char** io_ptr)
{
	EncoderData* usr_data = &(((GlzData*)usr)->data);
	return encoder_usr_more_space(usr_data, io_ptr);
}

static int glz_usr_more_lines(GlzEncoderUsrContext* usr, unsigned char** lines)
{
	EncoderData* usr_data = &(((GlzData*)usr)->data);
...
static int zlib_usr_more_space(ZlibEncoderUsrContext* usr, unsigned char** io_ptr)
{
	EncoderData* usr_data = &(((ZlibData*)usr)->data);
	return encoder_usr_more_space(usr_data, io_ptr);
}

static int zlib_usr_more_input(ZlibEncoderUsrContext* usr, unsigned char** input)
{
	EncoderData* usr_data = &(((ZlibData*)usr)->data);
	int buf_size;
...
static int jpeg_usr_more_space(JpegEncoderUsrContext* usr, unsigned char** io_ptr)
{
	EncoderData* usr_data = &(((JpegData*)usr)->data);
	return encoder_usr_more_space(usr_data, io_ptr);
}

static int jpeg_usr_more_lines(JpegEncoderUsrContext* usr, unsigned char** lines)
{
	EncoderData* usr_data = &(((JpegData*)usr)->data);
	return encoder_usr_more_lines(usr_data, lines);
}
...
{	
    ...
    pthread_mutex_init(&glz_drawables_inst_to_free_lock, NULL);
...
static GlzSharedDictionary* glz_shared_dictionary_new(unsigned char id, SharedDictionary* dict)
{
	if (dict == NULL) {
		return NULL;
	}
...
GlzSharedDictionary* ImageEncoders::create_glz_dictionary(unsigned char id, int window_size)
{
...
bool ImageEncoders::image_encoders_get_glz_dictionary(unsigned char id, int window_size)
{
...
bool ImageEncoders::image_encoders_glz_create(unsigned char id)
{	
	glz = new GlzEncoder();
	...
	compress_send_data_t* o_comp_data)
{
    ...
    pthread_rwlock_rdlock(&glz_dict->encode_lock);
...
{	
	...
	int comp_head_left;
	int stride;
    //unsigned char_t* lz_out_start_unsigned char;
    ...
    lz_out_start_unsigned char = lz_data->data.bufs_head->buf.unsigned chars + comp_head_filled;
...
ImageEncoders::~ImageEncoders()
{
    ...
    pthread_mutex_destroy(&glz_drawables_inst_to_free_lock);
