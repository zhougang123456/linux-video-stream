...
static void convert_RGB16_to_RGB24(void* line, int width, unsigned char** out_line)
{
...
static void convert_BGR24_to_RGB24(void* in_line, int width, unsigned char** out_line)
{
	int x;
    unsigned char* out_pix;
    unsigned char* line = (unsigned char*)in_line;
	if (!(out_line && *out_line)) {
		return;
	}
...
static void convert_BGRX32_to_RGB24(void* line, int width, unsigned char** out_line)
{
    unsigned int* src_line = (unsigned int *)line;
    unsigned char* out_pix;
	int x;
...
void JpegEncoder::do_jpeg_encode(unsigned char* lines, unsigned int num_lines)
{
    unsigned char* lines_end;
    unsigned char* RGB24_line;
	int stride, width;
	JSAMPROW row_pointer[1];
	width = cur_image.width;
	stride = cur_image.stride;

    RGB24_line = (unsigned char*)malloc(sizeof(unsigned char) * width * 3);
